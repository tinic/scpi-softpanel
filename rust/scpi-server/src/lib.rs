...
fn to_command(msg: ClientMessage) -> Command {
    match msg {
        ...
        ClientMessage::SetNull { enabled, value } => Command::SetNull { enabled, value },
        ClientMessage::SetAcBandwidth { hz } => Command::SetAcBandwidth(hz),
        ClientMessage::SetFreqAperture { seconds } => Command::SetFreqAperture(seconds),
